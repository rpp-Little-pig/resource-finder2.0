...
Resource Finder 是一个面向全网资源搜索获取的本地工具。用户输入影视、课程、软件、文档、素材等关键词后，系统会调用资源搜索接口在全网范围内检索可访问的资源线索，并把返回的链接、平台、提取码和说明自动整理成清晰卡片。

它的核心价值是“搜索全网资源并整理可用链接”：把接口返回的分散结果变成可打开、可复制、可筛选的资源列表，帮助用户更快找到需要的资料入口。

## 功能特点

- 全网资源搜索获取，适合查找影视、课程、软件、文档、素材等资源线索
- 本地 Web 界面，浏览器访问 `http://localhost:5177`
- 后端代理调用资源搜索接口，前端不会暴露 `Authorization`
- 自动清洗接口返回内容，识别并展示链接、平台、提取码和简短说明
...
本项目仅对接口检索到的全网资源线索和链接进行整理展示，不存储、不上传、不分发任何资源文件，也不保证第三方链接的有效性、合法性与安全性。请在遵守法律法规、平台规则和版权要求的前提下使用。
